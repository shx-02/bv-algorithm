...
ARITH_RULE`0 < N <=> 1 < 1 + N`;bitand;GSYM BITSET_EQ_BITSNUM];ALL_TAC] THEN
RULE_ASSUM_TAC(SIMP_RULE[NOT_LT;LE_LT]) THEN POP_ASSUM MP_TAC THEN
MP_TAC(ARITH_RULE`~(c = d:num) /\ d = 2 EXP N ==> ~(c = 2 EXP N)`) THEN
ASM_SIMP_TAC[] THEN REPEAT STRIP_TAC THEN SIMP_TAC[GSYM BITSET_EQ_BITSNUM] THEN
...
THEN SIMP_TAC[GSYM MULT_2;ARITH_RULE`2 * 2 EXP N = 2 EXP 1 * 2 EXP N `;GSYM EXP_ADD] THEN
ASM_SIMP_TAC[ARITH_RULE`1 + N = SUC N`] THEN
SUBGOAL_THEN`bitset (d - 2 EXP N + 2 EXP N) = bitset d`SUBST1_TAC THENL[
ASM_SIMP_TAC[ARITH_RULE`2 EXP N < d ==> d - 2 EXP N + 2 EXP N = d`];ALL_TAC] THEN
STRIP_TAC THEN SUBGOAL_THEN`bitset d INTER {N} = {N}`SUBST1_TAC THENL[
SIMP_TAC[INTER;IN_ELIM_THM;EXTENSION] THEN ASM_SIMP_TAC[IN_UNION] THEN SET_TAC[];ALL_TAC] THEN
ASM_CASES_TAC`c < 2 EXP N` THENL[SIMP_TAC[BITSET_EQ_BITSNUM] THEN
SUBGOAL_THEN`bits_of_num c INTER {N} = {}`SUBST1_TAC THENL[
MP_TAC(SPECL[`c:num`;`N:num`] (GSYM BITS_OF_NUM_SUBSET_NUMSEG_EQ)) THEN
ASM_SIMP_TAC[] THEN SET_TAC[LT_REFL];ALL_TAC] THEN
SIMP_TAC[UNION_EMPTY] THEN ASM_SIMP_TAC[] THEN SUBGOAL_THEN`{k | (1 <= k /\ k <= 2 EXP N) /\
      ...
      ODD
      (CARD (bits_of_num c INTER bits_of_num (k - 1)) *
       CARD (bitset (d - 2 EXP N) INTER bits_of_num (k - 1)))}`SUBST1_TAC THENL[
       SIMP_TAC[EXTENSION;IN_ELIM_THM] THEN GEN_TAC THEN EQ_TAC
THENL[SIMP_TAC[GSYM BITSET_EQ_BITSNUM] THEN ASM_SIMP_TAC[] THEN
SIMP_TAC[SET_RULE`(s UNION t) INTER u = s INTER u UNION t INTER u`] THEN STRIP_TAC THEN
POP_ASSUM MP_TAC THEN SUBGOAL_THEN`{N} INTER bitset (x- 1) = {}`SUBST1_TAC THENL[
ASSUME_TAC(SPEC`N:num` BITS_OF_NUM_POW2) THEN SIMP_TAC[BITSET_EQ_BITSNUM;GSYM DISJOINT] THEN
SIMP_TAC[DISJOINT_SING] THEN MP_TAC(SPECL[`x-1`;`N:num`] (GSYM BITS_OF_NUM_SUBSET_NUMSEG_EQ)) THEN
UNDISCH_TAC`x <= 2 EXP N` THEN UNDISCH_TAC`1 <= x` THEN
SIMP_TAC[ARITH_RULE`1 <= x ==> x <= 2 EXP N ==> x - 1  < 2 EXP N`] THEN
...
SUBGOAL_THEN`{N} INTER bitset (x- 1) = {}`SUBST1_TAC THENL[
ASSUME_TAC(SPEC`N:num` BITS_OF_NUM_POW2) THEN SIMP_TAC[BITSET_EQ_BITSNUM;GSYM DISJOINT] THEN
SIMP_TAC[DISJOINT_SING] THEN MP_TAC(SPECL[`x-1`;`N:num`] (GSYM BITS_OF_NUM_SUBSET_NUMSEG_EQ))
THEN UNDISCH_TAC`x <= 2 EXP N` THEN UNDISCH_TAC`1 <= x` THEN
SIMP_TAC[ARITH_RULE`1 <= x ==> x <= 2 EXP N ==> x - 1  < 2 EXP N`] THEN
...
SUBGOAL_THEN`{k | (1 <= k /\ k <= 2 EXP N) /\
      ODD(CARD (bitset c INTER bitset (k - 1)) *
       ...
       {k | (1 <= k /\ k <= 2 EXP N) /\
      ODD(CARD (bitset c INTER bitset (k - 1)) *
       (CARD (bitset (d - 2 EXP N) INTER bitset (k - 1)) + CARD{N}))}`SUBST1_TAC THENL[
...
SIMP_TAC[BITSET_EQ_BITSNUM;GSYM DISJOINT] THEN SIMP_TAC[DISJOINT_SING] THEN
MP_TAC(SPECL[`d - 2 EXP N`;`N:num`] (GSYM BITS_OF_NUM_SUBSET_NUMSEG_EQ))
...
SIMP_TAC[ARITH_RULE`c < 2 EXP N + 2 EXP N <=> c - 2 EXP N  < 2 EXP N`] THEN
SET_TAC[LT_REFL];ALL_TAC] THEN
SIMP_TAC[CARD_SING;ODD_MULT;ARITH_RULE`N + 1 = SUC N`;ODD;NOT_ODD;GSYM IN_NUMSEG] THEN
MP_TAC(SPECL[`N:num`;`c:num`] BITAND_ODD_CARD) THEN ASM_SIMP_TAC[ARITH_RULE`1 < N ==> 0 < N`] THEN
SIMP_TAC[FINITE_NUMSEG;FINITE_RESTRICT;SET_RULE`{x |x IN s /\ p x /\ q x} INTER
{x |x IN s /\ p x /\ ~q x} = {}`;GSYM CARD_UNION;GSYM NOT_ODD] THEN
SIMP_TAC[SET_RULE`{x |x IN s /\ p x /\ q x} UNION {x |x IN s /\ p x /\ ~q x} =
{x |x IN s /\ p x}`;bitand];ALL_TAC] THEN
RULE_ASSUM_TAC(SIMP_RULE[ARITH_RULE`~(c < 2 EXP N) <=> c = 2 EXP N \/ 2 EXP N < c `]) THEN
POP_ASSUM MP_TAC THEN STRIP_TAC THENL[
ASM_SIMP_TAC[BITSET_EQ_BITSNUM;BITS_OF_NUM_POW2;INTER_IDEMPOT] THEN
SIMP_TAC[SET_RULE`s INTER u UNION s = s`;SET_RULE`(s UNION {N}) INTER u UNION {N} =
s INTER u UNION {N}`;CARD_SING;MULT_CLAUSES] THEN
SUBGOAL_THEN`CARD
 {k | (1 <= k /\ k <= 2 EXP N) /\
      ODD
      (CARD ({N} INTER bits_of_num (k - 1)) *
...
THEN SIMP_TAC[EXTENSION;IN_ELIM_THM] THEN GEN_TAC THEN SIMP_TAC[IN_NUMSEG] THEN EQ_TAC THENL[STRIP_TAC THEN
POP_ASSUM MP_TAC THEN SUBGOAL_THEN`{N} INTER bits_of_num (x- 1) = {}`SUBST1_TAC THENL[
...
SIMP_TAC[CARD_CLAUSES;MULT_CLAUSES;ODD];ALL_TAC] THEN SET_TAC[];ALL_TAC] THEN
SUBGOAL_THEN`{k | (1 <= k /\ k <= 2 EXP N) /\
      ODD(CARD (bits_of_num (d - 2 EXP N) INTER bits_of_num (k - 1) UNION {N}))} =
      {k | (1 <= k /\ k <= 2 EXP N) /\
      ODD(CARD (bits_of_num (d - 2 EXP N) INTER bits_of_num (k - 1)) + CARD {N})}`SUBST1_TAC THENL[
      RULE_ASSUM_TAC(SIMP_RULE[ARITH_RULE`SUC N = 1 + N`;EXP_ADD;EXP_1;MULT_2]) THEN
      SIMP_TAC[EXTENSION;IN_ELIM_THM] THEN GEN_TAC THEN
REPEAT AP_TERM_TAC THEN MATCH_MP_TAC CARD_UNION THEN SIMP_TAC[FINITE_BITS_OF_NUM;FINITE_INTER;FINITE_SING]
THEN ASSUME_TAC(SPEC`N:num` BITS_OF_NUM_POW2) THEN SIMP_TAC[BITSET_EQ_BITSNUM;GSYM DISJOINT] THEN
...
SIMP_TAC[ARITH_RULE`c < 2 EXP N + 2 EXP N <=> c - 2 EXP N  < 2 EXP N`] THEN SET_TAC[LT_REFL];ALL_TAC] THEN
SIMP_TAC[CARD_SING;ARITH_RULE`N + 1 = SUC N`;ODD;NOT_ODD;GSYM IN_NUMSEG;GSYM BITSET_EQ_BITSNUM;GSYM bitand] THEN
 MP_TAC(SPECL[`N:num`;`d - 2 EXP N`] BITAND_ODD_CARD) THEN SIMP_TAC[ARITH_RULE`0 < c - a <=> a < c`;
 ARITH_RULE`n - 2 EXP N  < 2 EXP N  <=> n < 2 EXP N + 2 EXP N`] THEN
 SIMP_TAC[GSYM MULT_2;ARITH_RULE`2 * 2 EXP N = 2 EXP 1 * 2 EXP N `;GSYM EXP_ADD] THEN
ASM_SIMP_TAC[ARITH_RULE`1 + N = SUC N`;ARITH_RULE`1 < N ==> 0 < N`] THEN STRIP_TAC THEN
MP_TAC(SPECL[`N:num`;`d - 2 EXP N`] (GSYM BITAND_CARD_ODD_EQ_EVEN)) THEN
SIMP_TAC[ARITH_RULE`0 < c - a <=> a < c`] THEN SIMP_TAC[ARITH_RULE`n - 2 EXP N  < 2 EXP N
<=> n < 2 EXP N + 2 EXP N`] THEN SIMP_TAC[GSYM MULT_2;ARITH_RULE`2 * 2 EXP N = 2 EXP 1 * 2 EXP N `;
GSYM EXP_ADD] THEN ASM_SIMP_TAC[ARITH_RULE`1 + N = SUC N`;ARITH_RULE`1 < N ==> 0 < N`;ADD];ALL_TAC] THEN
MP_TAC (SPECL[`c - 2 EXP N`;`N:num`] BITSET_ADD) THEN SIMP_TAC[ARITH_RULE`n - 2 EXP N  < 2 EXP N
<=> n < 2 EXP N + 2 EXP N`] THEN SIMP_TAC[GSYM MULT_2;ARITH_RULE`2 * 2 EXP N = 2 EXP 1 * 2 EXP N `;
GSYM EXP_ADD] THEN ASM_SIMP_TAC[ARITH_RULE`1 + N = SUC N`] THEN ASM_SIMP_TAC[ARITH_RULE`2 EXP N < c
==> c - 2 EXP N + 2 EXP N = c`] THEN STRIP_TAC THEN SIMP_TAC[SET_RULE`(s UNION t) INTER t = t`] THEN
SUBGOAL_THEN`{k | (1 <= k /\ k <= 2 EXP N) /\
      ODD
...
THEN SIMP_TAC[EXTENSION;IN_ELIM_THM] THEN GEN_TAC THEN EQ_TAC THENL[
SIMP_TAC[] THEN STRIP_TAC THEN POP_ASSUM MP_TAC THEN SIMP_TAC[BITSET_EQ_BITSNUM] THEN
SUBGOAL_THEN`{N} INTER bits_of_num (x - 1) = {}`SUBST1_TAC THENL[
MP_TAC(SPECL[`x-1`;`N:num`] (GSYM BITS_OF_NUM_SUBSET_NUMSEG_EQ)) THEN
ASM_SIMP_TAC[ARITH_RULE`1 <= x ==> x <= a ==> x - 1 < a`] THEN SET_TAC[LT_REFL];ALL_TAC] THEN
SIMP_TAC[UNION_EMPTY];ALL_TAC] THEN SIMP_TAC[] THEN
STRIP_TAC THEN POP_ASSUM MP_TAC THEN SIMP_TAC[BITSET_EQ_BITSNUM] THEN
...
SUBGOAL_THEN`{k | (1 <= k /\ k <= 2 EXP N) /\
      ODD(CARD ((bitset (c - 2 EXP N) UNION {N}) INTER bitset (k - 1) UNION {N}) *
       ...
       {k | (1 <= k /\ k <= 2 EXP N) /\
      ODD(CARD (bitset (c - 2 EXP N) INTER bitset (k - 1) UNION {N}) *
       CARD (bitset (d - 2 EXP N) INTER bitset (k - 1) UNION {N}))}`SUBST1_TAC THENL[
       SIMP_TAC[SET_RULE`(s UNION t) INTER u = s INTER u UNION t INTER u`]
       THEN SIMP_TAC[EXTENSION;IN_ELIM_THM] THEN GEN_TAC
THEN EQ_TAC THENL[SIMP_TAC[] THEN STRIP_TAC THEN POP_ASSUM MP_TAC THEN
...
MP_TAC(SPECL[`x-1`;`N:num`] (GSYM BITS_OF_NUM_SUBSET_NUMSEG_EQ)) THEN
ASM_SIMP_TAC[ARITH_RULE`1 <= x ==> x <= a ==> x - 1 < a`] THEN SET_TAC[LT_REFL];ALL_TAC] THEN
SIMP_TAC[UNION_EMPTY];ALL_TAC] THEN
SUBGOAL_THEN`{k | (1 <= k /\ k <= 2 EXP N) /\
      ODD(CARD (bitset (c - 2 EXP N) INTER bitset (k - 1) UNION {N}) *
       CARD (bitset (d - 2 EXP N) INTER bitset (k - 1) UNION {N}))} =
       {k | (1 <= k /\ k <= 2 EXP N) /\
      ...
      (CARD (bitset (d - 2 EXP N) INTER bitset (k - 1)) + CARD {N}))}`SUBST1_TAC THENL[
      SIMP_TAC[EXTENSION;IN_ELIM_THM] THEN GEN_TAC THEN REPEAT AP_TERM_TAC THEN
      SUBGOAL_THEN`CARD (bitset (c - 2 EXP N) INTER bitset (x - 1) UNION {N})
...
SIMP_TAC[ARITH_RULE`c - 2 EXP N  < 2 EXP N <=> c < 2 EXP N + 2 EXP N`] THEN
SIMP_TAC[ARITH_RULE`SUC N = 1 + N`;EXP_ADD;EXP_1;MULT_2] THEN SET_TAC[LT_REFL];ALL_TAC] THEN
SUBGOAL_THEN`CARD (bitset (d - 2 EXP N) INTER bitset (x - 1) UNION {N})
...
SIMP_TAC[CARD_SING;ARITH_RULE`a + 1 = SUC a`;ODD_MULT;ODD] THEN
RULE_ASSUM_TAC(SIMP_RULE[ODD_MULT]) THEN
...
THEN ASM_SIMP_TAC[ARITH_RULE`0 < c - a <=> a < c`;ARITH_RULE`n - 2 EXP N  < 2 EXP N  <=>
n < 2 EXP N + 2 EXP N`;ARITH_RULE`2 EXP N < c /\ 2 EXP N < d /\ ~(c = d) ==>
~(c - 2 EXP N = d - 2 EXP N)`;bitand] THEN STRIP_TAC THEN SIMP_TAC[NOT_ODD;GSYM IN_NUMSEG] THEN
ONCE_SIMP_TAC[SET_RULE`{x | x IN s /\ p x /\ q x} = {x | x IN s /\ p x} DIFF
{x |x IN s /\ p x /\ ~q x}`] THEN SIMP_TAC[NOT_EVEN] THEN SIMP_TAC[FINITE_NUMSEG;FINITE_RESTRICT;
SET_RULE`{x |x IN s /\ p x /\ q x } SUBSET {x |x IN s /\ p x}`;CARD_DIFF] THEN
ASM_SIMP_TAC[GSYM bitand] THEN ONCE_REWRITE_TAC[TAUT`P /\ Q /\ R <=> P /\ R /\ Q`] THEN
ONCE_SIMP_TAC[SET_RULE`{x | x IN s /\ p x /\ q x} = {x | x IN s /\ p x} DIFF
{x |x IN s /\ p x /\ ~q x}`] THEN SIMP_TAC[NOT_EVEN] THEN SIMP_TAC[FINITE_NUMSEG;
FINITE_RESTRICT;SET_RULE`{x |x IN s /\ p x /\ q x } SUBSET {x |x IN s /\ p x}`;CARD_DIFF] THEN
ONCE_REWRITE_TAC[TAUT`P /\ Q /\ R <=> P /\ R /\ Q`] THEN ASM_SIMP_TAC[bitand;IN_NUMSEG] THEN
...
ASM_SIMP_TAC[ARITH_RULE`2 EXP (N - 2) < 2 EXP (N - 1) ==> 2 EXP (N - 2) + 2 EXP (N - 1)
-(2 EXP (N - 1) - 2 EXP (N - 2)) = 2 EXP (N - 2) + 2 EXP (N - 2)`] THEN
...
(`!k:num.1 <= k /\ k <= dimindex(:(2,N)finite_pow) /\
         1 <= i /\ i <= dimindex(:(2,N)finite_pow) /\
        1 <= j /\ j <= dimindex(:(2,N)finite_pow)==>
(project (qbasis k:(N)qstate):complex^(2,N)finite_pow^(2,N)finite_pow)$i$j = if (i = j) /\ (i = k) then Cx(&1) else Cx(&0)`,
SIMP_TAC[project;qouter1;QBASIS_COMPONENT;LAMBDA_BETA] THEN SIMP_TAC[COND_LMUL;COND_CNJ;CNJ_CX;COND_RMUL] THEN
...
(`!k:num. 1 <= k /\ k <= dimindex (:(2,N)finite_pow) ==>
   (project (qbasis k:(N)qstate)) ** project (qbasis k) = project (qbasis k)`,
  GEN_TAC THEN SIMP_TAC[cmatrix_mul;LAMBDA_BETA;CART_EQ;PROJECT_QBASIS_COMPONENT] THEN
...
(`cmat_sum (1..dimindex(:(2,N)finite_pow)) (\i. project(qbasis i:(N)qstate)) =
  id_cmatrix:complex^(2,N)finite_pow^(2,N)finite_pow`,
  SIMP_TAC[cmat_sum;id_cmatrix;LAMBDA_BETA;CART_EQ] THEN REPEAT STRIP_TAC THEN
  AP_THM_TAC THEN AP_TERM_TAC THEN ASM_SIMP_TAC[PROJECT_QBASIS_COMPONENT] THEN
...
REPEAT STRIP_TAC THEN SIMP_TAC[state_after_ora;IDCMAT_MUL_QSTATE] THEN AP_TERM_TAC THEN
...
REPEAT STRIP_TAC THEN SIMP_TAC[second_h;STATE_AFTER_ORAC;zero_h;NHADAMARD_MUL_ZEROQSTATE] THEN
...
THEN SIMP_TAC[REAL_ARITH`a * b + c * b = (a + c) * b:real`] THEN SIMP_TAC[REAL_OF_NUM_ADD;FINITE_RESTRICT;FINITE_NUMSEG;GSYM CARD_UNION;
...
`encryption_1 = xor pixel_1 keys`;;

let encryption_2 = new_definition
`encryption_2 = xor pixel_2 keys`;;

let encryption_3 = new_definition
`encryption_3 = xor pixel_3 keys`;;

let encryption_4 = new_definition
`encryption_4 = xor pixel_4 keys`;;

let decryption_1 = new_definition
`decryption_1 = xor (binarysum(encryption_1)) keys`;;

let decryption_2 = new_definition
`decryption_2 = xor (binarysum(encryption_2)) keys`;;

let decryption_3 = new_definition
`decryption_3 = xor (binarysum(encryption_3)) keys`;;

let decryption_4 = new_definition
`decryption_4 = xor (binarysum(encryption_4)) keys`;;
